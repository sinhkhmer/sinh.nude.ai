(function ($) {
    ...
    function landingFade() {
        if ($('[data-mb="landing-fade"]').length) {
            var container = $('[data-mb="landing-fade"]');

            if (/Mobi|Android/.test(navigator.userAgent)) {
                setTimeout(function () {
                    var loadObjectTop = container.offset().top + container.innerHeight();
                    var loadWindowBottom = $(window).scrollTop() + $(window).innerHeight();

                    if ($(window).innerHeight() < loadWindowBottom) {
                        if (loadObjectTop <= loadWindowBottom) {
                            container.addClass('fade-nude');
                        }
                    }
                }, 500);

                document.addEventListener('scroll', function () {
                    var objectTop = container.offset().top + container.innerHeight();
                    var windowBottom = $(window).scrollTop() + $(window).innerHeight();

                    if ($(window).innerHeight() < windowBottom) {
                        if (objectTop < windowBottom) {
                            container.addClass('fade-nude');
                        } else {
                            container.removeClass('fade-nude');
                        }
                    } else {
                        container.removeClass('fade-nude');
                    }
                ...
                container.hover(function (e) {
                    ...
                    $(this).toggleClass('fade-click');
                });
            } else {
                var loadObjectTop = $('[data-mb="landing-fade"]').offset().top;
                var loadWindowTop = $(window).scrollTop();

                if (loadObjectTop <= loadWindowTop) {
                    container.addClass('fade-nude');
                }

                $(window).scroll(function () {
                    var objectTop = $('[data-mb="landing-fade"]').offset().top;
                ...
                container.on('mouseenter', function (e) {
                    ...
                    $(this).addClass('fade-click');
                }).on('mouseleave', function (e) {
                    e.preventDefault();

                    $(this).removeClass('fade-click');
                });
            }
        }
    }
    ...
    function serviceFade() {
        if ($('[data-mb="scroll-fade"]').length) {
            var container = $('[data-mb="scroll-fade"]');

            if (/Mobi|Android/.test(navigator.userAgent)) {
                document.addEventListener('scroll', function () {
                    container.each(function () {
                        var objectTop = $(this).offset().top + $(this).innerHeight();
                        var windowBottom = $(window).scrollTop() + $(window).innerHeight();

                        if (objectTop < windowBottom) {
                            if (objectTop < windowBottom) {
                                $(this).addClass('fade-nude');
                            } else {
                                $(this).removeClass('fade-nude');
                            }
                        } else {
                            $(this).removeClass('fade-nude').removeClass('fade-click');
                        }
                    });
                });
                container.each(function () {
